...
import { WEAPON_TYPE_MAPPING, ATTACK_TYPE_MAPPING } from '../../../models/category';
...
export class DataViz2Component implements OnInit {
    ...
    private addTitle() {
        this.svg.append('text')
            ...
            .text('Évolution des attaques terroristes aux États-Unis (1990-2015)');
    }
    ...
    private getData() {
        ...
        this.dataService.data$.subscribe((data) => {
            if (data) {
                const groupedData = this.dataService.groupDataByYear(data, this.currentDataField);
    ...
    private updateChart() {
        const filteredData = this.currentData.filter(d => d.year >= this.startYear);
    ...
    private createScales(data: YearEntry[]): Scales {
        ...
        const categories = data[0]?.counts.map(c => WEAPON_TYPE_MAPPING[c['field']] || ATTACK_TYPE_MAPPING[c['field']]) || [];
        const color = d3
            .scaleOrdinal<string>()
            .domain(categories)
    ...
    private drawLines(data: YearEntry[]) {
        ...
        categories.forEach(category => {
            const lineGenerator = d3.line<YearEntry>()
                .x(d => this.scales!.x(d.year))
                .y(d => {
                    const countEntry = d.counts.find(c => c['field'] === category);
                    return this.scales!.y(countEntry ? countEntry.count : 0);
                })
                .curve(d3.curveMonotoneX);
    
            const translated_category = WEAPON_TYPE_MAPPING[category] || ATTACK_TYPE_MAPPING[category];
    ...
    private addHoverEffect(data: YearEntry[]) {
        ...
        this.svg.append('rect')
            ...
            .on('mousemove', (event: MouseEvent) => {
                ...
                const [mouseXPos, mouseYPos] = d3.pointer(event, chartContainer);
    ...
    private generateTooltipContent(yearData: YearEntry): string {
        const sortedCounts = [...yearData.counts].sort((a, b) => b.count - a.count);
        ...
        sortedCounts.forEach(item => {
            ...
            content += `
                ...
                >
                    <strong>
                    <span class="tooltip-category">${WEAPON_TYPE_MAPPING[item['field']] || ATTACK_TYPE_MAPPING[item['field']]}</span>
                    <span class="tooltip-value">${item.count} <strong>(${percentage}%) </span>
                    </strong>
                </div>
